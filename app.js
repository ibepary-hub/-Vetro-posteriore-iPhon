...
async function connectItRicambi(){const user=document.getElementById("itRicambiLoginUser")?.value.trim(),pass=document.getElementById("itRicambiLoginPass")?.value||"",btn=document.getElementById("itRicambiLoginSubmit"),res=document.getElementById("itRicambiResult");if(!user||!pass){res.textContent="Inserisci email/username e password IT-Ricambi.";return;}btn.disabled=true;btn.textContent="Accesso…";try{const d=await callItRicambi({action:"login",username:user,password:pass});itRicambiSession={connected:true,expires_at:d.expires_at||null};document.getElementById("itRicambiLoginPass").value="";setItRicambiLoginBox(false);renderItRicambiSession();res.innerHTML=`<strong>Account IT-Ricambi collegato.</strong><span>Da ora il gestionale riutilizza questa sessione finché resta valida.</span>`;}catch(e){res.textContent=e?.message||"Login IT-Ricambi non riuscito.";}finally{btn.disabled=false;btn.textContent="Accedi a IT-Ricambi";}}
