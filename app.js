...
function ensureItRicambiModal(){let m=document.getElementById("itRicambiCostModal");if(m)return m;document.body.insertAdjacentHTML("beforeend",`<div id="itRicambiCostModal" class="saleModal" hidden><div class="saleDialog itRicambiDialog" role="dialog" aria-modal="true"><div class="saleDialogHead"><div><span class="saleKicker">SOLO ADMIN</span><h2>IT-Ricambi · costo prodotto</h2></div><button id="itRicambiClose" class="closeBtn" type="button">×</button></div><div id="itRicambiItem" class="saleItemLabel"></div><label class="fieldLabel">Link prodotto IT-Ricambi</label><input id="itRicambiUrl" class="modalInput" type="url" placeholder="https://www.it-ricambi.com/..."><div class="itRicambiActions"><button id="itRicambiFetch" class="primaryAction" type="button">Cerca prezzo</button><a id="itRicambiOpen" class="miniBtn" target="_blank" rel="noopener noreferrer" hidden>Apri scheda</a></div><div id="itRicambiResult" class="itRicambiResult">Incolla il link esatto del ricambio.</div><div class="itRicambiApplyRow"><label><span>Prezzo trovato</span><input id="itRicambiPrice" class="modalInput" inputmode="decimal" readonly></label><button id="itRicambiApply" class="primaryAction" type="button" disabled>Usa come costo</button></div><div class="itRicambiNotice">Il prezzo IT-Ricambi viene conservato separatamente dal tuo costo effettivo. “Usa come costo” lo copia solo quando lo decidi tu.</div></div></div>`);m=document.getElementById("itRicambiCostModal");document.getElementById("itRicambiClose").onclick=()=>m.hidden=true;m.onclick=e=>{if(e.target===m)m.hidden=true;};return m;}
let itRicambiPending=null;
function openItRicambiCost(key,label){if(!isAdmin())return;const m=ensureItRicambiModal(),q=supplierQuote(key);itRicambiPending={key,label,quote:q};document.getElementById("itRicambiItem").innerHTML=`<strong>${escapeHtml(label)}</strong><span>Fonte costo: IT-Ricambi</span>`;document.getElementById("itRicambiUrl").value=q?.source_url||"";document.getElementById("itRicambiPrice").value=q?.displayed_price!=null?String(q.displayed_price).replace(".",","):"";const res=document.getElementById("itRicambiResult");res.innerHTML=q?`<strong>${escapeHtml(q.source_title||"IT-Ricambi")}</strong><span>Ultimo controllo: ${new Date(q.checked_at).toLocaleString("it-IT")}</span>`:"Incolla il link esatto del ricambio.";const open=document.getElementById("itRicambiOpen");if(q?.source_url){open.href=q.source_url;open.hidden=false;}else open.hidden=true;document.getElementById("itRicambiApply").disabled=!(q?.displayed_price>=0);m.hidden=false;bindItRicambiActions();}
function bindItRicambiActions(){const fetchBtn=document.getElementById("itRicambiFetch"),applyBtn=document.getElementById("itRicambiApply");if(fetchBtn?.dataset.bound!=="1"){fetchBtn.dataset.bound="1";fetchBtn.onclick=fetchItRicambiPrice;}if(applyBtn?.dataset.bound!=="1"){applyBtn.dataset.bound="1";applyBtn.onclick=applyItRicambiPrice;}}
async function fetchItRicambiPrice(){if(!isAdmin()||!itRicambiPending)return;const url=document.getElementById("itRicambiUrl").value.trim(),btn=document.getElementById("itRicambiFetch"),res=document.getElementById("itRicambiResult");if(!url){res.textContent="Inserisci il link IT-Ricambi.";return;}btn.disabled=true;btn.textContent="Controllo…";res.textContent="Lettura prezzo da IT-Ricambi…";try{const {data,error}=await sb.functions.invoke("beparytech-itricambi-price",{body:{url}});if(error||data?.error)throw new Error(data?.error||error?.message||"Prezzo non disponibile");const quote={source_url:data.url||url,source_title:data.title||"IT-Ricambi",displayed_price:Number(data.price),currency:data.currency||"EUR",checked_at:data.checked_at||new Date().toISOString(),supplier:"IT-Ricambi"};const {error:saveErr}=await sb.from("beparytech_supplier_quotes").upsert({workspace_owner_id:workspaceOwnerId,item_key:itRicambiPending.key,supplier:"IT-Ricambi",source_url:quote.source_url,source_title:quote.source_title,displayed_price:quote.displayed_price,currency:quote.currency,checked_at:quote.checked_at,updated_by:currentUser.id,updated_at:new Date().toISOString()},{onConflict:"workspace_owner_id,item_key,supplier"});if(saveErr)throw saveErr;supplierQuotes[itRicambiPending.key]=quote;itRicambiPending.quote=quote;document.getElementById("itRicambiPrice").value=String(quote.displayed_price).replace(".",",");res.innerHTML=`<strong>${escapeHtml(quote.source_title)}</strong><span>Prezzo trovato: ${eur(quote.displayed_price)} · ${new Date(quote.checked_at).toLocaleString("it-IT")}</span>`;const open=document.getElementById("itRicambiOpen");open.href=quote.source_url;open.hidden=false;document.getElementById("itRicambiApply").disabled=false;refreshCostUi();}catch(e){res.textContent=e?.message||"Non riesco a leggere automaticamente questo prezzo.";}finally{btn.disabled=false;btn.textContent="Cerca prezzo";}}
